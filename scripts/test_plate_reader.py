...
def main() -> None:
    ...
    if not recortes:
        print("No hay recortes de placa para probar OCR.")
    ...
    for ruta in recortes[:5]:
        resultado = leer_placa_desde_recorte(str(ruta))
        reporte = registrar_reporte_ocr(resultado, fuente)
        print("-" * 72)
        print(f"Recorte: {ruta}")
        print(f"Procesada: {resultado['preprocesamiento'].get('ruta_imagen_procesada')}")
        print(f"Caracteres segmentados: {resultado['cantidad_caracteres_segmentados']}")
        print(f"Estado: {resultado['estado']}")
        print(f"Mensaje: {resultado['mensaje']}")
        print(f"Reporte JSON: {reporte['ruta_json']}")
